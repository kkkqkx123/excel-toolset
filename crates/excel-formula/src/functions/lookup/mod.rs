...
pub(crate) fn consume_range_marker(args: &[CellValue], start: usize) -> Option<(usize, usize, usize)> {
    if start >= args.len() {
        return None;
    }
...
use self::{
    vlookup::lookup_vlookup,
    xlookup::lookup_xlookup,
    hlookup::lookup_hlookup,
    index_match::lookup_index,
    index_match::lookup_match,
    offset_indirect::lookup_offset,
    offset_indirect::lookup_indirect,
    row_column::lookup_row,
    row_column::lookup_column,
    row_column::lookup_rows,
    row_column::lookup_columns,
    choose_address::lookup_choose,
    choose_address::lookup_address,
};
