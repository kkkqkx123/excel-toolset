...
pub fn parse_cell_ref(ref_str: &str) -> Result<(u32, u16)> {
    ...
    let col_end = ref_str
        ...
        .ok_or_else(|| format!("Invalid cell reference: {}", ref_str))?;

    let col_part = &ref_str[..col_end];
    let row_part = &ref_str[col_end..];

    if col_part.is_empty() || row_part.is_empty() {
        return Err(format!("Invalid cell reference: {}", ref_str).into());
    }

    let col = col_to_index(col_part)?;
    let row: u32 = row_part
        .parse()
        .map_err(|_| format!("Invalid row number: {}", row_part))?;
...
pub fn parse_range(range_str: &str) -> Result<(u32, u16, u32, u16)> {
    ...
    if parts.len() != 2 {
        return Err(format!("Invalid range: {}", range_str).into());
    }
...
pub fn col_to_index(col: &str) -> Result<u16> {
    ...
    if col.is_empty() {
        return Err("Empty column reference".into());
    }

    let mut result: u16 = 0;
    for c in col.chars() {
        if !c.is_ascii_uppercase() {
            return Err(format!("Invalid column character: {}", c).into());
        }
        result = result
            .checked_mul(26)
            .and_then(|v| v.checked_add((c as u16) - ('A' as u16) + 1))
            .ok_or_else(|| format!("Column overflow: {}", col))?;
    }
...
pub fn index_to_col(mut idx: u16) -> String {
    ...
    loop {
        let rem = idx % 26;
        result.insert(0, char::from_u32(('A' as u16 + rem) as u32).unwrap());
        if idx < 26 {
            break;
        }
        idx = idx / 26 - 1;
